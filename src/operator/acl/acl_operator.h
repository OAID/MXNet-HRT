 ...
 * \author Joey
*/
#ifndef CAFFE_ACL_OPERATOR_HPP_
#define CAFFE_ACL_OPERATOR_HPP_
...
public:
    ...
    explicit ACLOperator(bool is_gpu);
    virtual ~ACLOperator();
    ...
    inline void setConvMethod(){ convolution_method_hint_=ConvolutionMethodHint::DIRECT;}
...
template <typename Dtype>
Dtype* GetDataPtr(ACLOperator* op,const std::vector<TBlob> &data,int idx){
    return data[idx].dptr<Dtype>();
}

template <typename Dtype>
Dtype* InputdataPtr(ACLOperator* op,const std::vector<TBlob> &data,int idx=-1){
    if (idx==-1) idx=0;
    op->input_idx()=idx;
    return GetDataPtr<Dtype>(op, data,idx);
}
template <typename Dtype>
Dtype* OutputdataPtr(ACLOperator* op,const std::vector<TBlob> &data,int idx=-1){
    if (idx==-1) idx=0;
    op->output_idx()=idx;
    return GetDataPtr<Dtype>(op, data,idx);
}
...
template <typename Dtype>
void acl_run(ACLOperator* op,const std::vector<TBlob> &in_data,const std::vector<TBlob> &out_data,bool multi_input_run=true){
    if (multi_input_run) {
        op->acl_run(InputdataPtr<Dtype>(op,in_data,op->input_idx()),OutputdataPtr<Dtype>(op,out_data,op->output_idx()));
        return ;
    }
    for (int i = 0; i < in_data.size(); ++i) {
        op->tensor_mem(op->cinput(i),InputdataPtr<Dtype>(op,in_data,i));
    }
    op->acl_run(NULL,OutputdataPtr<Dtype>(op,out_data,op->output_idx()));
}
}  // namespace op
}  // namespace mxnet
...
template <> \
Dtype* InputdataPtr(ACLOperator* op,const std::vector<TBlob> &data,int index); \
template <> \
Dtype* OutputdataPtr(ACLOperator* op,const std::vector<TBlob> &data,int index); \
template <> \
Dtype* GetDataPtr(ACLOperator* op,const std::vector<TBlob> &data,int index); \
